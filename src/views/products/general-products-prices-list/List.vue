<template>
  <default-layout>
    <div class="price-lists-page">
      <PageHeader :icon="priceListsIcon" title-key="pages.GeneralProductsPriceLists.title"
        description-key="pages.GeneralProductsPriceLists.description" />
      ...
      <div class="bg-gray-50 rounded-md -mx-6">
        <div class="flex flex-wrap items-center justify-end gap-3 border-y border-y-slate-300 px-4 sm:!px-6 py-3">
        ...
        <div v-if="showAdvancedFilters"
          ...
          <div class="flex flex-wrap gap-3 flex-1">
            ...
            <TextInput v-model="filterListName" density="comfortable" variant="outlined" hide-details
              :placeholder="t('pages.GeneralProductsPriceLists.list.filters.listName')" class="flex-1 bg-white" @keyup.enter="applyFilters" />
...
const handleEdit = (item: any) => {
  router.push({ name: 'GeneralProductPriceListEdit', params: { id: item.id } })
}
...
const confirmStatusChange = async () => {
  ...
  try {
    ...
    toast.success(
      t('pages.GeneralProductsPriceLists.list.messages.statusChangeSuccess', {
    ...
    toast.error(err?.response?.data?.message || t('pages.GeneralProductsPriceLists.list.messages.statusChangeError'))
  } finally {
...
const fetchPriceLists = async (append = false) => {
  try {
    ...
    toast.error(err?.response?.data?.message || t('pages.GeneralProductsPriceLists.list.messages.fetchError'))
  } finally {
    loading.value = false
...
const updateHeadersOnServer = async () => {
  try {
    ...
    toast.error(err?.response?.data?.message || t('pages.GeneralProductsPriceLists.list.messages.columnsUpdateError'))
  } finally {
