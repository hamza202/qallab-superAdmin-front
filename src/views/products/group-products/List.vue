...
const subItemHeaders = ref<TableHeader[]>([])
...
const itemToChangeStatus = ref<GroupProduct | null>(null)

// Sub Item Status change dialog
const showSubItemStatusChangeDialog = ref(false)
const subItemStatusChangeLoading = ref(false)
const subItemToChangeStatus = ref<SubItem | null>(null)
const parentItemId = ref<number | null>(null)

// Delete dialog
const showDeleteDialog = ref(false)
const deleteLoading = ref(false)
const itemToDelete = ref<GroupProduct | null>(null)
...
// === Icons ===
const groupProductsIcon = `<svg width="43" height="43" viewBox="0 0 43 43" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const computedSubItemHeaders = computed(() => {
  if (subItemHeaders.value.length > 0) {
    return subItemHeaders.value.map(header => ({
      key: header.key,
      title: header.title,
      width: '150px',
    }))
...
const fetchData = async (cursor?: string | null, append = false) => {
  try {
    ...
    if (append) {
      ...
      // Initialize sub item headers
      if (response.sub_item_headers) {
        subItemHeaders.value = response.sub_item_headers.filter(h => h.key !== 'actions')
      }
...
const handleView = (item: any) => {
  router.push({ name: 'GroupProductView', params: { id: item.id } })
}

const handleEdit = (item: any) => {
  router.push({ name: 'GroupProductEdit', params: { id: item.id } })
}
...
// Handle item status change - open confirmation dialog
const handleStatusChange = (item: any) => {
  itemToChangeStatus.value = { ...item }
  showStatusChangeDialog.value = true
}

// Confirm item status change after dialog
...
// Handle sub_item status change - open confirmation dialog
const handleSubItemStatusChange = (subItem: any) => {
  // Find parent item
  const parentItem = tableItems.value.find(item => 
    item.sub_items.some(sub => sub.id === subItem.id)
  )
  if (parentItem) {
    parentItemId.value = parentItem.id
  }
  subItemToChangeStatus.value = { ...subItem }
  showSubItemStatusChangeDialog.value = true
}

// Confirm sub_item status change after dialog
const confirmSubItemStatusChange = async () => {
  if (!subItemToChangeStatus.value) return

  try {
    subItemStatusChangeLoading.value = true
    ...
    if (parentItemId.value) {
      const parentIndex = tableItems.value.findIndex(t => t.id === parentItemId.value)
      if (parentIndex !== -1) {
        const subIndex = tableItems.value[parentIndex].sub_items.findIndex(
          sub => sub.id === subItemToChangeStatus.value!.id
        )
        if (subIndex !== -1) {
          tableItems.value[parentIndex].sub_items[subIndex].is_active = newStatus
        }
      }
    }
  } catch (err: any) {
    ...
    showSubItemStatusChangeDialog.value = false
    subItemToChangeStatus.value = null
...
const openCreate = () => {
  router.push({ name: 'GroupProductsCreate' })
}
...
<template>
  <default-layout>
    ...
    <StatusChangeDialog v-model="showStatusChangeDialog" :loading="statusChangeLoading"
      :item-name="itemToChangeStatus?.name" :current-status="itemToChangeStatus?.status"
      @confirm="confirmStatusChange" />

    <!-- Status Change Confirmation Dialog (Sub Item) -->
    <StatusChangeDialog v-model="showSubItemStatusChangeDialog" :loading="subItemStatusChangeLoading"
      :item-name="subItemToChangeStatus?.name" :current-status="subItemToChangeStatus?.is_active"
      @confirm="confirmSubItemStatusChange" />
  </default-layout>
</template>
