...
import { GridIcon, trash_1_icon, trash_2_icon, importIcon, columnIcon, exportIcon, searchIcon, linkIcon, printerIcon } from "@/components/icons/globalIcons";
...
const handlePrint = (item: { id?: string | number; uuid?: string }) => {
  ...
  const routeData = router.resolve({ name: 'QuotationsFuelsPrint', params: { id: uuid } });
...
const handleDownloadPdf = async (item: { id?: string | number; uuid?: string }) => {
  ...
  try {
    const body = (await api.get(
      `/purchases/quotations/fuels/${uuid}/pdf`
...
const fetchList = async (append = false) => {
  ...
  try {
    ...
    error(err?.response?.data?.message || t('purchases.shared.lists.quotation.messages.fetchError'));
  } finally {
    loading.value = false;
...
const handleView = (item: { id?: string | number; uuid?: string }) => {
  const uuid = item.uuid ?? String(item.id);
  router.push({ name: 'QuotationsFuelsView', params: { id: uuid } });
};
...
const confirmDelete = async () => {
  ...
  try {
    ...
    await api.delete(`/purchases/quotations/fuels/${uuid}`);
    success(t('purchases.shared.lists.quotation.messages.deleteSuccess'));
    showDeleteDialog.value = false;
    itemToDelete.value = null;
    await fetchList();
  } catch (err: any) {
    console.error('Error deleting quotation:', err);
    error(err?.response?.data?.message || t('purchases.shared.lists.quotation.messages.deleteError'));
  } finally {
...
const confirmBulkDelete = async () => {
  try {
    deleteLoading.value = true;
    await api.post('/purchases/quotations/fuels/bulk-delete', {
      ids: selectedRequests.value,
    });
    success(
      t('purchases.shared.lists.quotation.messages.bulkDeleteSuccess', {
...
<template>
  <default-layout>
    <div class="pricesOffers-page">
      <PageHeader :icon="GridIcon" title-key="pages.QuotationsFuels.title"
        description-key="pages.QuotationsFuels.description" />
    ...
    <DeleteConfirmDialog
      ...
      :title="t('purchases.shared.lists.quotation.dialogs.delete.title')"
      :message="t('purchases.shared.lists.quotation.dialogs.delete.message')"
    ...
    <DeleteConfirmDialog
      ...
      :loading="deleteLoading"
      :title="t('purchases.shared.lists.quotation.dialogs.bulkDelete.title')"
      :message="t('purchases.shared.lists.quotation.dialogs.bulkDelete.message', { count: selectedRequests.length })"
