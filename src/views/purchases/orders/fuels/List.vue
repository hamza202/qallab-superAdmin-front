...
const fetchList = async () => {
  ...
  try {
    ...
    const url = params.toString()
      ? `/purchases/orders/fuels?${params.toString()}`
      : '/purchases/orders/fuels';
...
const confirmDelete = async () => {
  ...
  try {
    ...
    await api.delete(`/purchases/orders/fuels/${uuid}`);
...
const handleView = (item: any) => {
    router.push({ name: "OrdersFuelsView", params: { id: item.uuid } });
};
...
const confirmBulkDelete = async () => {
  try {
    deleteLoading.value = true;
    await api.post('/purchases/orders/fuels/bulk-delete', {
...
<template>
  <default-layout>
    <div class="pricesOffers-page">
      <PageHeader
        :icon="GridIcon"
        title-key="pages.OrdersFuels.title"
        description-key="pages.OrdersFuels.description"
      />
    ...
    <StatusChangeFeature
      ...
      :change-status-url="`/purchases/orders/fuels/${itemToChangeStatus?.uuid}/change-status`"
      @success="fetchList"
    />
  </default-layout>
</template>
