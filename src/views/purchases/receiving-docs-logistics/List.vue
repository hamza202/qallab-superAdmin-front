...
const fetchList = async (append = false) => {
  ...
  try {
    ...
    const url = `/purchases/receiving-logistic-docs?${params.toString()}`;
...
const confirmDelete = async (item: { id?: string | number }) => {
  try {
    ...
    await api.delete(`/purchases/receiving-logistic-docs/${String(item.id)}`);
...
const openCreate = () => {
  ...
  router.push({ name: "ReceivingDocsLogisticsCreate" });
};
...
const confirmBulkDelete = async () => {
  try {
    deleteLoading.value = true;
    await api.post("/purchases/receiving-logistic-docs/bulk-delete", {
...
<template>
  <default-layout>
    <div class="receiving-logistic-docs-page">
      <PageHeader
        :icon="GridIcon"
        title-key="pages.ReceivingDocsLogistics.title"
        description-key="pages.ReceivingDocsLogistics.description"
      />
