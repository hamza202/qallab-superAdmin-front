...
const fetchList = async (cursor?: string | null, append = false) => {
    try {
        ...
        const url = `/purchases/invoices?${params.toString()}`;
...
const handleView = (item: { id?: string | number; uuid?: string }) => {
    ...
    router.push({ name: 'PurchaseInvoicesView', params: { id: uuid } });
};

const handleEdit = (item: { id?: string | number; uuid?: string }) => {
    const uuid = item.uuid ?? String(item.id);
    router.push({ name: 'PurchaseInvoicesEdit', params: { id: uuid } });
};

const handlePrint = (item: { id?: string | number; uuid?: string }) => {
    const uuid = item.uuid ?? String(item.id);
    const route = router.resolve({ name: 'PurchaseInvoicesPrint', params: { id: uuid } });
...
const confirmDelete = async (item: { uuid?: string; id?: string | number } & Partial<InvoiceItem>) => {
    ...
    try {
        await api.delete(`/purchases/invoices/${uuid}`);
...
const confirmBulkDelete = async () => {
    try {
        ...
        await api.post('/purchases/invoice/bulk-delete', {
...
<template>
    <default-layout>
        ...
        <StatusChangeFeature v-model="showChangeStatusDialog" :item="itemToChangeStatus"
            :change-status-url="`/purchases/invoices/${itemToChangeStatus?.uuid}/change-status`"
            :title="t('purchases.shared.statusChange.title')" :message="t('purchases.shared.statusChange.message')" @success="fetchList" />
    </default-layout>
</template>
