...
const TABLE_NAME = 'admin_purchases_invoices_building_materials';
const {
    allHeaders,
    shownHeaders,
    updatingHeaders,
...
const fetchList = async (cursor?: string | null, append = false) => {
    try {
        ...
        const url = `/purchases/invoices/building-materials?${params.toString()}`;
...
const confirmDelete = async (item: { uuid?: string; id?: string | number } & Partial<InvoiceItem>) => {
    ...
    try {
        await api.delete(`/purchases/invoices/building-materials/${uuid}`);
...
const confirmBulkDelete = async () => {
    try {
        ...
        await api.post('/purchases/invoices/building-materials/bulk-delete', {
...
<template>
    <default-layout>
        ...
        <StatusChangeFeature v-model="showChangeStatusDialog" :item="itemToChangeStatus"
            :change-status-url="`/purchases/invoices/building-materials/${itemToChangeStatus?.uuid}/change-status`"
