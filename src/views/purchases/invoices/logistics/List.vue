...
const fetchList = async (cursor?: string | null, append = false) => {
    try {
        ...
        const url = `/purchases/invoices/logistics?${params.toString()}`;
        ...
        error(err?.response?.data?.message || t('purchases.invoices.list.messages.fetchError'));
    } finally {
        loading.value = false;
...
const handleView = (item: { id?: string | number; uuid?: string }) => {
    ...
    router.push({ name: 'PurchaseInvoicesLogisticsView', params: { id: uuid } });
};

const handleEdit = (item: { id?: string | number; uuid?: string }) => {
    const uuid = item.uuid ?? String(item.id);
    router.push({ name: 'PurchaseInvoicesLogisticsEdit', params: { id: uuid } });
};

const handlePrint = (item: { id?: string | number; uuid?: string }) => {
    const uuid = item.uuid ?? String(item.id);
    const route = router.resolve({ name: 'PurchaseInvoicesLogisticsPrint', params: { id: uuid } });
...
const confirmDelete = async (item: { uuid?: string; id?: string | number } & Partial<InvoiceItem>) => {
    ...
    try {
        await api.delete(`/purchases/invoices/logistics/${uuid}`);
        success(t('purchases.invoices.list.messages.deleteSuccess'));
        await fetchList();
    } catch (err: any) {
        console.error('Error deleting invoice:', err);
        error(err?.response?.data?.message || t('purchases.invoices.list.messages.deleteError'));
    } finally {
    }
};
...
const confirmBulkDelete = async () => {
    try {
        ...
        await api.post('/purchases/invoices/logistics/bulk-delete', {
            ids: selectedInvoices.value,
        });
        success(
            t('purchases.invoices.list.messages.bulkDeleteSuccess', {
...
const openCreateInvoice = () => {
    router.push({ name: 'PurchaseInvoicesLogisticsCreate' });
};
...
<template>
    <default-layout>
        <div class="purchase-invoices-page">
            <PageHeader :icon="GridIcon" title-key="pages.PurchaseInvoicesLogistics.title"
                description-key="pages.PurchaseInvoicesLogistics.description" />
                ...
                <div :class="hasSelectedInvoices ? 'justify-between' : 'justify-end'"
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <ButtonWithIcon variant="flat" color="primary-100" height="40" rounded="4"
                            ...
                            :prepend-icon="plusIcon" :label="t('purchases.invoices.list.buttons.add')" @click="openCreateInvoice" />
                    </div>
                </div>
        ...
        <DeleteConfirmDialog
            ...
            :title="t('purchases.invoices.list.dialogs.bulkDelete.title')"
            :message="t('purchases.invoices.list.dialogs.bulkDelete.message', { count: selectedInvoices.length })"
        ...
        <StatusChangeFeature
            ...
            :change-status-url="`/purchases/invoices/logistics/${itemToChangeStatus?.uuid}/change-status`"
