            ...
            <div>
                ...
                <div class="flex flex-wrap items-center gap-3 mb-6 text-sm px-6">
                    ...
                    <router-link to="/purchases/requests/logistics/list" class="text-gray-600 hover:text-primary-600">
                        {{ t('purchases.views.shared.listRequestsLogistics') }}
                    </router-link>
                    <span class="text-lg text-gray-300">/</span>
                ...
                <div class="p-6">
                    <h2 class="text-lg font-bold text-primary-900 mb-6">{{ t('purchases.views.shared.basicDataSectionAlt') }}</h2>
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        <v-divider vertical class="my-6"></v-divider>
                        <div class="info-item-bordered flex-1 px-6 py-4">
                            <label class="font-semibold text-sm text-gray-500 mb-2 block">{{ t('purchases.shared.forms.common.labels.requestDate') }}</label>
                            <p class="text-base font-semibold text-gray-900">{{ logisticsData?.request_datetime || '—'
                            }}</p>
                        </div>
                        <v-divider vertical class="my-6"></v-divider>
                        <div class="info-item-bordered flex-1 px-6 py-4">
                            <label class="font-semibold text-sm text-gray-500 mb-2 block">{{ t('purchases.requests.logistics.form.labels.transportCompany') }}</label>
                    ...
                    <div v-for="(detail, index) in logisticsDetails" :key="index" class="p-4">
                        <div class="flex flex-wrap gap-4">
                            ...
                            <div class="info-item-bordered flex-1 px-6 py-4">
                                ...
                                <p class="text-base font-semibold text-gray-900">{{ t('purchases.views.shared.vehiclesWithUnit', { n: detail.transport_no }) }}</p>
                            </div>
                            <v-divider vertical class="my-6"></v-divider>
                            <div class="info-item-bordered flex-1 px-6 py-4">
                ...
                <div>
                    <div class="bg-primary-50 px-6 py-4">
                        <div class="flex items-center gap-2 text-primary-900 font-bold">
                            ...
                            <h2 class="text-base">{{ t('purchases.views.shared.tabs.movedMaterials') }}</h2>
                        </div>
                    </div>
...
const fetchLogisticsData = async () => {
    ...
    try {
        ...
        error(e?.response?.data?.message || t('purchases.views.shared.loadLogisticsRequestError'))
    } finally {
...
const getCategoryName = (categories: number[]) => {
    ...
    return categories.map(categoryId => {
        const categoryItem = categoriesItems.value.find((row: any) => row.value == categoryId)
...
const getAmPmLabel = (interval: string | null | undefined) => {
    if (!interval) return '—'
    ...
    return (
        intervalObj?.label ??
        (interval === 'am'
            ? t('purchases.views.shared.timeInterval.am')
            : interval === 'pm'
              ? t('purchases.views.shared.timeInterval.pm')
              : interval === 'both'
                ? t('purchases.views.shared.timeInterval.both')
                : interval)
    )
}
...
// Tabs configuration (reserved if tab UI is added)
const tabs = computed(() => [
    {
    ...
    {
        title: t('purchases.views.shared.tabs.transportDetails'),
        value: 1,
    ...
    {
        title: t('purchases.views.shared.tabs.movedMaterials'),
        value: 2,
...
const logisticsHeaders = computed(() => [
    { title: t('purchases.views.shared.materialTypeShort'), key: 'material_type' },
    { title: t('purchases.requests.logistics.form.detailCard.tripCount'), key: 'trip_no' },
    { title: t('purchases.requests.logistics.form.detailCard.executionDuration'), key: 'execution_interval' },
    { title: t('purchases.views.shared.workHoursTransport'), key: 'am_pm' },
    { title: t('purchases.views.shared.fromDateLabel'), key: 'from_date' },
    { title: t('purchases.views.shared.toDateLabel'), key: 'to_date' },
    { title: t('purchases.requests.logistics.form.detailCard.vehicleType'), key: 'transport_type' },
    { title: t('purchases.requests.logistics.form.detailCard.vehicleCount'), key: 'transport_no' },
    { title: t('purchases.requests.logistics.form.detailCard.loadingResponsible'), key: 'loading_party' },
    { title: t('purchases.requests.logistics.form.detailCard.unloadingResponsible'), key: 'downloading_party' },
])

const itemHeaders = computed(() => [
    { title: t('purchases.requests.materialProduct.form.tableHeaders.productName'), key: 'item_name' },
    { title: t('purchases.requests.materialProduct.form.tableHeaders.unit'), key: 'unit_name' },
    { title: t('purchases.requests.materialProduct.form.tableHeaders.quantity'), key: 'quantity' },
])
