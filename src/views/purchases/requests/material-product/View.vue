            ...
            <div>
                ...
                <div class="flex flex-wrap items-center gap-3 mb-6 text-sm px-6">
                    ...
                    <router-link to="/purchases/requests/material-product/list"
                        ...
                        {{ t('purchases.views.shared.listRequestsMaterial') }}
                    </router-link>
                    <span class="text-lg text-gray-300">/</span>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600">{{ t('purchases.views.shared.requestMaterialSubtitle') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="mb-8">
                            <div class="flex flex-wrap gap-3 items-center justify-between bg-primary-50 px-6 py-3">
                                <div class="flex items-center gap-2 text-primary-900">
                                    ...
                                    <h2 class="text-lg font-bold ">{{ t('purchases.requests.materialProduct.form.transportService') }}</h2>
                                </div>
                            </div>
...
const requestType = computed(() => {
    ...
    if (type === 'purchase_request_without_logistics') return t('purchases.views.shared.requestTypeWithoutLogistics')
    if (type === 'purchase_request_with_logistics') return t('purchases.views.shared.requestTypeWithLogistics')
    return type || ''
})
...
const productItems = computed(() => {
    ...
    return requestData.value.items.map((item: any) => ({
        ...
        notes: item.notes || t('purchases.views.shared.noNotes'),
    }))
})
...
const transportServiceItems = computed(() => {
    ...
    return [{
        ...
        notes: logistics.notes || t('purchases.views.shared.noNotes'),
    }]
})
...
const ServicesHeaders = computed(() => [
    { title: t('purchases.requests.logistics.form.labels.projectName'), key: 'project_location' },
    { title: t('purchases.views.shared.fromDateLabel'), key: 'from_date' },
    { title: t('purchases.views.shared.toDateLabel'), key: 'to_date' },
    { title: t('purchases.requests.materialProduct.form.tableHeaders.vehicleTypes'), key: 'vehicle_type' },
    { title: t('purchases.requests.materialProduct.form.tableHeaders.transportTiming'), key: 'trip_time' },
    { title: t('purchases.shared.forms.common.tableHeaders.notes'), key: 'notes' },
])
