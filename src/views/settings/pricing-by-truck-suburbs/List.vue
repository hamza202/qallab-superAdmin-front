...
const fetchPricings = async (append = false) => {
    try {
        ...
        const url = `/zone-city-suburb-km-truck-pricings?${queryString}`;
...
const handleEditPricing = (item: any) => {
    router.push(`/settings/pricing-by-truck-suburbs/edit/${item.id}`);
};

const handleViewPricing = (item: any) => {
    router.push(`/settings/pricing-by-truck-suburbs/view/${item.id}`);
};

const handleDeletePricing = async (item: any) => {
    try {
        await api.delete(`/zone-city-suburb-km-truck-pricings/${item.id}`);
...
const confirmStatusChange = async () => {
    ...
    try {
        ...
        await api.patch(`/zone-city-suburb-km-truck-pricings/${itemToChangeStatus.value.id}/change-status`, { status: newStatus });
...
const confirmBulkDelete = async () => {
    ...
    try {
        ...
        await api.post('/zone-city-suburb-km-truck-pricings/bulk-delete', { ids: selectedPricings.value });
...
<template>
    <default-layout>
        <div class="pricing-by-truck-page">
            <PageHeader :icon="SettingsIcon" title-key="pages.pricingByTruckSuburbs"
                description-key="pages.pricingByTruckSuburbsDescription" />
            <div
