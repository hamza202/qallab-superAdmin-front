...
const fetchPricingData = async () => {
    ...
    try {
        loading.value = true;
        const res = await api.get<any>(`/zone-city-suburb-km-truck-pricings/${route.params.id}`);
        if (res.data) {
            const d = res.data;
            ...
            form.transport_type = d.transport_type ?? null;
...
const handleSave = async (navigateTo: 'tracks' | 'list') => {
    ...
    try {
        ...
        if (isEditing.value) {
            await api.put(`/zone-city-suburb-km-truck-pricings/${route.params.id}`, payload);
            toast.success('تم تحديث التسعير بنجاح');
        } else {
            await api.post('/zone-city-suburb-km-truck-pricings', payload);
            toast.success('تم إضافة التسعير بنجاح');
        }

        if (navigateTo === 'list') {
            router.push('/settings/pricing-by-truck-suburbs/list');
...
<template>
    <default-layout>
        <div class="pricing-form-page -mx-6">
            <AppFormBreadcrumb list-path="/settings/pricing-by-truck-suburbs/list" module-root-key="الإعدادات"
                list-label-key="إدارة الأحياء" create-label-key="إضافة تسعيرة بالشاحنة للأحياء" edit-label-key="تعديل تسعيرة بالشاحنة للأحياء"
                :is-edit-mode="isEditing" />

            <TopHeader :icon="coins" title-key="إضافة تسعيرة بالشاحنة للأحياء" description-key="تمكنك من إضافة تسعيرة بالشاحنة للأحياء"
                :show-action="false" />
                ...
                <v-form ref="formRef" v-model="isFormValid" @submit.prevent>
                    ...
                    <div v-else class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                            ...
                            <TextInput v-model="form.max_distance_km" label="أقصى مسافة" placeholder="50" type="number"
                            ...
                            <SelectWithIconInput v-model="form.transport_type" label="نوع الشاحنة" placeholder="اختر النوع"
                                :items="transportTypes" :rules="[required()]"
                                :error-messages="formErrors['transport_type']"
                                @update:model-value="delete formErrors['transport_type']" density="comfortable" />

                            <TextInput v-model="form.transports_no" label="عدد الشاحنات" placeholder="2" type="number"
                            ...
                            <SelectWithIconInput v-model="form.trip_time_slot" label="وقت الرحلة" placeholder="صباحاً"
                                :items="tripTimeSlots" :rules="[required()]"
                                :error-messages="formErrors['trip_time_slot']"
                            ...
                            <PriceInput v-model="form.price_amount" label="السعر" placeholder="200" type="number"
                                :rules="[required()]" showRialIcon :error-messages="formErrors['price_amount']"
                            ...
                            <SelectWithIconInput v-model="form.road_type" label="نوع الطريق" placeholder="اختر النوع"
                                ...
                                :error-messages="formErrors['road_type']"
                        ...
                        <div class="flex gap-3 justify-center pt-4 sm:w-[75%] mx-auto mt-12">
                            ...
                            <ButtonWithIcon variant="flat" color="primary" height="44" rounded="4"
                                custom-class="font-semibold text-base px-12 flex-1"
                                label="حفظ والعودة إلى قائمة التسعيرة بالشاحنة" @click="handleSave('list')" :loading="saving"
                                :disabled="saving" />
