...
const loadingBrandData = ref(false);
...
const fetchBrandData = async (brandId: number) => {
  try {
    loadingBrandData.value = true;
    const response = await api.get(`/brands/${brandId}`);
    const brand = response.data;

    form.id = brand.id;
    form.nameAr = brand.name_translations?.ar || brand.name;
    form.nameEn = brand.name_translations?.en || brand.name;
...
<template>
  <AppDialog v-model="internalOpen" title="إضافة علامة تجارية" :max-width="520">
    ...
    <v-form ref="formRef" v-model="isFormValid" @submit.prevent>
      <div v-if="loadingBrandData" class="flex justify-center items-center py-12">
