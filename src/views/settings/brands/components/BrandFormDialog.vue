...
watch(
  ...
  (open) => {
    ...
    if (props.brand) {
      form.nameAr = props.brand.nameAr ?? "";
      form.nameEn = props.brand.nameEn ?? "";
      form.logo = (props.brand.logo as File[] | null) ?? null;
      form.notes = props.brand.notes ?? "";
      form.status = props.brand.status ?? true;
    } else {
      resetForm();
    }
  }
  ...
  <AppDialog v-model="internalOpen" title="إضافة علامة تجارية" :max-width="520">
    ...
    <v-form ref="formRef" v-model="isFormValid" @submit.prevent>
      ...
      <div class="flex justify-center mb-4">
        <button type="button" class="flex items-center gap-1 text-sm font-semibold text-primary-600">
      ...
      <div class="mb-4">
        <FileUploadInput v-model="form.logo" label="صورة الشعار" :multiple="false" :max-files="1" />
      </div>
