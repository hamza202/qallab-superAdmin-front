...
const codesIcon = `<svg width="52" height="52" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const typeItems = [
  ...
  { title: "أحرف وأرقام", value: "alphanumeric" },
];
...
const handleSave = async () => {
  ...
  isSaving.value = true;
  try {
    console.log("Saving code settings", { ...form });
  } finally {
    ...
    <div class="bg-gray-50 rounded-md p-4 sm:p-6">
      ...
      <v-form>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-y-3 gap-x-4 mb-4">
          ...
          <TextInput v-model="form.prefix" label="بادئة الكود" placeholder="بادئة الكود" :hide-details="false" />

          <TextInput v-model="form.digits" label="عدد الخانات" placeholder="عدد الخانات" type="number"
            :hide-details="false" />

          <TextInput v-model="form.startFrom" label="يبدأ من" placeholder="يبدأ من" type="number"
            :hide-details="false" />

          <div class="md:col-span-2 flex items-center md:justify-start gap-2">
            <CheckboxInput v-model="form.applyToExisting" :hide-details="true" />
