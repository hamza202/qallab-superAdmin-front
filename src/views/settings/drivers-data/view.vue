<template>
    <default-layout>
        <div class="driver-view-page -mx-6" :dir="pageDir">
            ...
            <div v-else-if="error" class="flex flex-col items-center justify-center min-h-[400px] px-6">
                ...
                <ButtonWithIcon color="primary" :label="t('common.ui.retry')" @click="fetchDriver" />
            </div>
            ...
            <div v-else-if="driver">
                <nav class="flex flex-wrap items-center gap-3 mb-6 text-sm px-6" aria-label="Breadcrumb">
                    ...
                    <span class="text-lg text-gray-300">/</span>
                    <router-link :to="{ name: 'DriversDataList' }" class="text-gray-600 hover:text-primary-600">
                        {{ t('pages.driversData.title') }}
                    </router-link>
                    <span class="text-lg text-gray-300">/</span>
                ...
                <div class="bg-white px-6 py-4 border-y border-gray-200">
                    <div class="flex items-center gap-4">
                        <div class="w-16 h-16 bg-primary-50 rounded-lg flex items-center justify-center shrink-0">
                            <span v-html="profileIcon" class="text-primary-600"></span>
                        </div>
                        <div>
                            <h1 class="text-lg font-bold text-gray-900 mb-1">{{ driver.name || '—' }}</h1>
                            <p class="text-sm text-gray-600">{{ licenseTypeValue }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <h2 class="text-xl font-bold text-primary-900 pb-4">
                        {{ t('pages.driversData.view.driverDataSection') }}
                    </h2>

                    <div class="flex flex-wrap gap-0">
                        <ViewField :label="t('pages.driversData.view.fields.name')" :value="driver.name" />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField :label="t('pages.driversData.view.fields.licenseNumber')" :value="driver.license_number" />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField :label="t('pages.driversData.view.fields.nationalId')" :value="driver.national_id" />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.licenseType')"
                            :value="licenseTypeValue"
                        />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.licenseExpiresAt')"
                            :value="formatDate(driver.license_expires_at)"
                        />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.medicalExamDate')"
                            :value="formatDate(driver.medical_exam_date)"
                        />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.hazardousMaterial')"
                            :value="driver.hazardous_material_qualified ? t('common.options.yes') : t('common.options.no')"
                            :value-class="
                                driver.hazardous_material_qualified ? 'text-green-600 font-semibold' : 'text-gray-900'
                            "
                        />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.status')"
                            :value="driver.is_active ? t('common.status.active') : t('common.status.inactive')"
                            :value-class="driver.is_active ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'"
                        />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.country')"
                            :value="driver.country?.name || '—'"
                        />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.logisticsCompany')"
                            :value="driver.logistics_company?.name || '—'"
                        />
                        <v-divider vertical class="hidden sm:block my-4 border-gray-200" />
                        <ViewField
                            :label="t('pages.driversData.view.fields.rating')"
...
const ViewField = defineComponent({
    ...
    setup(props) {
        return () =>
            h('div', { class: 'info-item-bordered flex-1 min-w-[180px] px-4 py-3' }, [
...
interface DriverDetail {
    ...
    logistics_company?: { id: number; name: string }
}
...
const profileIcon = `<svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const licenseTypeValue = computed(() => {
    const d = driver.value
    if (!d) return '—'
    if (d.license_type_label) return d.license_type_label
    if (d.license_type_labels?.length) return d.license_type_labels.join('، ')
    return d.license_type?.join(', ') || '—'
})
...
const fetchDriver = async () => {
    try {
        ...
        const response = await api.get<{ status?: boolean; data?: DriverDetail; message?: string }>(`/drivers/${id}`)
        const raw: any = response
        const data = raw?.data !== undefined ? raw.data : raw
        if (data && typeof data === 'object' && 'id' in data) {
            driver.value = data as DriverDetail
        } else {
