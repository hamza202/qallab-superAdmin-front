...
/** From zone suburbs: zone_city_suburb_id; optional legacy zone_city_track_id */
...
const fetchPricingData = async () => {
    ...
    try {
        loading.value = true;
        const res = await api.get<any>(`/zone-city-suburb-ton-pricings/${route.params.id}`);
        if (res.data) {
            const d = res.data;
...
const handleSave = async (navigateTo: 'tracks' | 'list') => {
    ...
    try {
        ...
        if (isEditing.value) {
            await api.put(`/zone-city-suburb-ton-pricings/${route.params.id}`, payload);
            toast.success('تم تحديث التسعير بنجاح');
        } else {
            await api.post('/zone-city-suburb-ton-pricings', payload);
            toast.success('تم إضافة التسعير بنجاح');
        }

        if (navigateTo === 'list') {
            router.push('/settings/pricing-per-ton-suburbs/list');
...
<template>
    <default-layout>
        <div class="pricing-per-ton-form-page -mx-6">
            <AppFormBreadcrumb list-path="/settings/pricing-per-ton-suburbs/list" module-root-key="الإعدادات"
                list-label-key="إدارة الأحياء" create-label-key="إضافة تسعيرة بالطن للأحياء" edit-label-key="تعديل تسعيرة بالطن للأحياء"
                :is-edit-mode="isEditing" />

            <TopHeader :icon="coins" title-key="إضافة تسعيرة بالطن للأحياء" description-key="تمكنك من إضافة تسعيرة بالطن للأحياء"
                :show-action="false" />
