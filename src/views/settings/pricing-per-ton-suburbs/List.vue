...
const fetchData = async (append = false) => {
    try {
        ...
        const response = await api.get<any>(`/zone-city-suburb-ton-pricings?${params.toString()}`);
...
const handleEdit = (item: any) => {
    router.push(`/settings/pricing-per-ton-suburbs/edit/${item.id}`);
};

const handleView = (item: any) => {
    router.push(`/settings/pricing-per-ton-suburbs/view/${item.id}`);
};

const handleDelete = async (item: any) => {
    try {
        await api.delete(`/zone-city-suburb-ton-pricings/${item.id}`);
...
const confirmStatusChange = async () => {
    ...
    try {
        ...
        await api.patch(`/zone-city-suburb-ton-pricings/${itemToChangeStatus.value.id}/change-status`, { status: newStatus });
...
const confirmBulkDelete = async () => {
    ...
    try {
        ...
        await api.post('/zone-city-suburb-ton-pricings/bulk-delete', { ids: selectedItems.value });
...
<template>
    <default-layout>
        <div class="pricing-per-ton-page">
            <PageHeader :icon="SettingsIcon" title-key="pages.pricingPerTonSuburbs"
                description-key="pages.pricingPerTonSuburbsDescription" />
