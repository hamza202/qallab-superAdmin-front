...
const fetchData = async (append = false) => {
    try {
        ...
        const response = await api.get<any>(`/zone-city-suburb-custom-pricings?${params.toString()}`);
...
const handleEdit = (item: any) => {
    router.push(`/settings/custom-pricing-suburbs/edit/${item.id}`);
};

const handleView = (item: any) => {
    router.push(`/settings/custom-pricing-suburbs/view/${item.id}`);
};

const handleDelete = async (item: any) => {
    try {
        await api.delete(`/zone-city-suburb-custom-pricings/${item.id}`);
...
const confirmStatusChange = async () => {
    ...
    try {
        ...
        await api.patch(`/zone-city-suburb-custom-pricings/${itemToChangeStatus.value.id}/change-status`, { status: newStatus });
...
const confirmBulkDelete = async () => {
    ...
    try {
        ...
        await api.post('/zone-city-suburb-custom-pricings/bulk-delete', { ids: selectedItems.value });
...
<template>
    <default-layout>
        <div class="custom-pricing-page">
            <PageHeader :icon="SettingsIcon" title-key="التسعير المخصص للأحياء"
                description-key="إدارة التسعيرات المخصصة للأحياء" />
