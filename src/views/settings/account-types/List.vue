...
import AccountTypeFormDialog from "@/views/settings/account-types/components/AccountTypeFormDialog.vue";
...
const accountTypesIcon = `<svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const editingAccountType = ref<any | null>(null);

const openCreateAccountType = () => {
  editingAccountType.value = null;
  showAccountTypeDialog.value = true;
};

const handleEditAccountType = (item: any) => {
...
const handleDeleteAccountType = (item: any) => {
  accountTypesTableItems.value = accountTypesTableItems.value.filter((t) => t.id !== item.id);
};

const handleSaveAccountType = (payload: any) => {
  if (editingAccountType.value && editingAccountType.value.id) {
    const index = accountTypesTableItems.value.findIndex((t) => t.id === editingAccountType.value.id);
    if (index !== -1) {
      accountTypesTableItems.value[index] = {
        ...accountTypesTableItems.value[index],
        name: payload.nameAr,
    ...
    const nextId = accountTypesTableItems.value.length
      ? Math.max(...accountTypesTableItems.value.map((t) => t.id)) + 1
      : 1;

    accountTypesTableItems.value.push({
      id: nextId,
      name: payload.nameAr,
      nameEn: payload.nameEn,
      role: payload.role,
...
const selectedAccountTypeIds = ref<(string | number)[]>([]);
...
const handleSelectAccountType = (item: any, selected: boolean) => {
  if (selected) {
    if (!selectedAccountTypeIds.value.includes(item.id)) {
      selectedAccountTypeIds.value.push(item.id);
    }
  } else {
    selectedAccountTypeIds.value = selectedAccountTypeIds.value.filter((id) => id !== item.id);
  }
};

const handleSelectAllAccountTypes = (selected: boolean) => {
  if (selected) {
    selectedAccountTypeIds.value = accountTypesTableItems.value.map((item) => item.id);
  } else {
    selectedAccountTypeIds.value = [];
  }
};

const hasSelectedAccountTypes = computed(() => selectedAccountTypeIds.value.length > 0);
...
<template>
  <default-layout>
    <div class="account-types-page">
      <PageHeader :icon="accountTypesIcon" title-key="pages.accountTypes.title"
        description-key="pages.accountTypes.description" />
      ...
      <div class="bg-gray-50 rounded-md -mx-6">
        <div :class="hasSelectedAccountTypes ? 'justify-between' : 'justify-end'"
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <ButtonWithIcon variant="flat" color="primary-100" height="40" rounded="4"
              ...
              :prepend-icon="plusIcon" :label="t('common.form.addNew')" @click="openCreateAccountType" />
          </div>
        </div>
        ...
        <div v-if="showAdvancedFilters"
          ...
          <div class="flex flex-wrap gap-3 flex-1 order-1 sm:order-2 justify-end sm:justify-start">
            <v-text-field v-model="filterName" density="comfortable" variant="outlined" hide-details
              placeholder="اسم الحساب" class="w-full sm:w-60 bg-white" />
            <v-text-field v-model="filterRole" density="comfortable" variant="outlined" hide-details placeholder="الدور"
        ...
        <DataTable :show-view="false" :headers="accountTypesTableHeaders" :items="accountTypesTableItems" show-actions show-checkbox
          @edit="handleEditAccountType" @delete="handleDeleteAccountType" @select="handleSelectAccountType"
          @selectAll="handleSelectAllAccountTypes" />

        <AccountTypeFormDialog v-model="showAccountTypeDialog" :account-type="editingAccountType"
          @save="handleSaveAccountType" />
      </div>
    </div>
  </default-layout>
</template>
