...
const confirmBulkDelete = async () => {
  ...
  try {
    ...
    toast.success(t('common.messages.general.bulkDeleteSuccess', { count: selectedUnits.value.length }));
    selectedUnits.value = [];
    await fetchUnits();
  } catch (err: any) {
...
<template>
  <default-layout>
        ...
        <div :class="hasSelectedUnits ? 'justify-between' : 'justify-end'"
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <ButtonWithIcon v-if="canCreate" variant="flat" color="primary-100" height="40" rounded="4"
              ...
              :prepend-icon="plusIcon" :label="t('common.form.addNew')" @click="openCreateUnit" />
          </div>
        </div>
    ...
    <DeleteConfirmDialog v-model="showDeleteDialog" :loading="deleteLoading" :title="$t('pages.units.list.deleteDialog.title')"
      :message="$t('pages.units.list.deleteDialog.message', { count: selectedUnits.length })" @confirm="confirmBulkDelete" />
