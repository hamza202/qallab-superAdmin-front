...
const fetchPricingData = async () => {
    ...
    try {
        loading.value = true;
        const res = await api.get<any>(`/zone-km-truck-pricings/${route.params.id}`);
        if (res.data) {
            const d = res.data;
...
const handleSave = async (navigateTo: 'tracks' | 'list') => {
    ...
    try {
        ...
        if (isEditing.value) {
            await api.put(`/zone-km-truck-pricings/${route.params.id}`, payload);
            toast.success('تم تحديث التسعير بنجاح');
        } else {
            await api.post('/zone-km-truck-pricings', payload);
    ...
    <default-layout>
        <div class="pricing-form-page -mx-6">
            <AppFormBreadcrumb list-path="/settings/pricing-by-truck/list" module-root-key="الإعدادات"
                list-label-key="إدارة المسارات المركزية" create-label-key="إضافة تسعيرة بالشاحنة" edit-label-key="تعديل تسعيرة بالشاحنة"
