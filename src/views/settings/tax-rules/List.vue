...
const selectedTaxRuleIds = ref<(string | number)[]>([]);
...
const hasSelectedTaxRules = computed(() => selectedTaxRuleIds.value.length > 0);
...
const handleSelectTaxRule = (item: any, selected: boolean) => {
  if (selected) {
    if (!selectedTaxRuleIds.value.includes(item.id)) {
      selectedTaxRuleIds.value.push(item.id);
    }
  } else {
    selectedTaxRuleIds.value = selectedTaxRuleIds.value.filter((id) => id !== item.id);
  }
};

const handleSelectAllTaxRules = (selected: boolean) => {
  if (selected) {
    selectedTaxRuleIds.value = taxRulesTableItems.value.map((item) => item.id);
  } else {
    selectedTaxRuleIds.value = [];
  }
};
  ...
  <default-layout>
    <div class="tax-rules-page">
      ...
      <div class="bg-gray-50 rounded-md -mx-6 ">
        <div :class="hasSelectedTaxRules ? 'justify-between' : 'justify-end'"
          ...
          <div class="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
            ...
            <div class="flex flex-wrap gap-3 flex-1 order-1 sm:order-2 justify-end sm:justify-start">
              <v-text-field v-model="filterName" density="comfortable" variant="outlined" hide-details
                placeholder="الاسم" class="w-full sm:w-52 bg-white" />
              <v-select v-model="filterType" density="comfortable" variant="outlined" hide-details
                :items="['النوع 1', 'النوع 2']" placeholder="النوع" class="w-full sm:w-40 bg-white" />
              <v-select v-model="filterStatus" density="comfortable" variant="outlined" hide-details
                :items="['نشطة', 'غير نشطة']" placeholder="الحالة" class="w-full sm:w-40 bg-white" />
            </div>
          </div>
        </div>

        <DataTable :headers="taxRulesTableHeaders" :items="taxRulesTableItems" show-actions show-checkbox
          @edit="handleEditTaxRule" @delete="handleDeleteTaxRule" @select="handleSelectTaxRule"
          @selectAll="handleSelectAllTaxRules" />
