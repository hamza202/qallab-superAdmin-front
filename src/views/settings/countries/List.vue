...
import CountryFormDialog from "@/views/settings/countries/components/CountryFormDialog.vue";
...
const countriesIcon = `<svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const itemToChangeStatus = ref<Country | null>(null);
...
const hasSelectedCountries = computed(() => selectedCountries.value.length > 0);

// Infinite scroll
const loadMoreTrigger = ref<HTMLElement | null>(null);

// Fetch countries from API
const fetchCountries = async (append = false) => {
  try {
    if (append) {
...
const confirmStatusChange = async () => {
  ...
  try {
    ...
    await api.patch(`/countries/${itemToChangeStatus.value.id}/change-status`, { status: newStatus });

    toast.success(`تم ${newStatus ? 'تفعيل' : 'تعطيل'} الدولة بنجاح`);
...
const confirmBulkDelete = async () => {
  ...
  try {
    ...
    await api.post('/countries/bulk-delete', { ids: selectedCountries.value });
    toast.success(`تم حذف ${selectedCountries.value.length} دولة بنجاح`);
    selectedCountries.value = [];
    await fetchCountries();
  } catch (err: any) {
    console.error('Error deleting countries:', err);
    toast.error(err?.response?.data?.message || 'Failed to delete countries');
  } finally {
...
const handleSelectCountry = (item: any, selected: boolean) => {
  if (selected) {
    if (!selectedCountries.value.includes(item.id)) {
      selectedCountries.value.push(item.id);
    }
  } else {
    selectedCountries.value = selectedCountries.value.filter((id) => id !== item.id);
  }
};

const handleSelectAllCountries = (selected: boolean) => {
  if (selected) {
    selectedCountries.value = tableItems.value.map((item) => item.id);
  } else {
    selectedCountries.value = [];
  }
};
...
<template>
  <default-layout>
    <div class="countries-page">
      <PageHeader :icon="countriesIcon" title-key="pages.countries.title" description-key="pages.countries.description" />
      <div
      ...
      <div class="bg-gray-50 rounded-md -mx-6">
        <div :class="hasSelectedCountries ? 'justify-between' : 'justify-end'"
          class="flex flex-wrap items-center gap-3 border-y border-y-slate-300 px-4 sm:px-6 py-3">

          <!-- Bulk Actions -->
          <div v-if="hasSelectedCountries"
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <ButtonWithIcon v-if="canCreate" variant="flat" color="primary-100" height="40" rounded="4"
              ...
              :prepend-icon="plusIcon" :label="t('common.addNew')" @click="openCreateCountry" />
          </div>
        </div>
          ...
          <div class="flex flex-wrap gap-3 justify-between">
            <div class="flex gap-3 flex-wrap">
              <TextInput v-model="filterName" density="comfortable" variant="outlined" hide-details
                placeholder="اسم الدولة" class="w-full sm:w-40 bg-white" @keyup.enter="applyFilters" />
        ...
        <DataTable :headers="tableHeaders" :items="tableItems" :loading="loading" show-checkbox show-actions
          @delete="handleDeleteCountry" @edit="handleEditCountry" @select="handleSelectCountry" @selectAll="handleSelectAllCountries"
    ...
    <DeleteConfirmDialog v-model="showDeleteDialog" :loading="deleteLoading" title="حذف الدول"
      :message="`هل أنت متأكد من حذف ${selectedCountries.length} دولة؟`" @confirm="confirmBulkDelete" />
    ...
    <!-- Country Form Dialog -->
    <CountryFormDialog v-model="showCountryDialog" :country-id="editingCountryId" @saved="handleSaveCountry" />
  </default-layout>
</template>
