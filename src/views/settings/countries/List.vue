...
const confirmBulkDelete = async () => {
  ...
  try {
    ...
    toast.success(t('common.messages.general.bulkDeleteSuccess', { count: selectedCountries.value.length }));
...
<template>
  <default-layout>
        ...
        <div :class="hasSelectedCountries ? 'justify-between' : 'justify-end'"
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <ButtonWithIcon v-if="canCreate" variant="flat" color="primary-100" height="40" rounded="4"
              ...
              :prepend-icon="plusIcon" :label="t('common.form.addNew')" @click="openCreateCountry" />
          </div>
        </div>
    ...
    <DeleteConfirmDialog v-model="showDeleteDialog" :loading="deleteLoading" :title="$t('pages.countries.list.deleteDialog.title')"
      :message="$t('pages.countries.list.deleteDialog.message', { count: selectedCountries.length })" @confirm="confirmBulkDelete" />
