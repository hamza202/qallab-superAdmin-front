...
const demoData: CustomPricing[] = [
  {
    id: 1,
    central_location: 'مجمع دركات',
    city: 'جدة',
    direction: 'شمال',
    min_distance_km: 50,
    max_distance_km: 100,
    material_type: 'مواد بناء',
    truck_type: 'قلاب طن 2 ساعات',
    truck_number: 'أ ب ج 1234',
    weight_ton: 200,
    price_per_unit: 2000,
    is_active: true,
  ...
  },
  {
    ...
    max_distance_km: 80,
    material_type: 'رمل',
    truck_type: 'شاحنة كبيرة',
    truck_number: 'د هـ و 5678',
    weight_ton: 150,
    price_per_unit: 1500,
    is_active: true,
    actions: { can_update: true, can_delete: true, can_change_status: true }
  },
  {
    ...
    max_distance_km: 60,
    material_type: 'حصى',
    truck_type: 'قلاب متوسط',
    truck_number: 'ز ح ط 9012',
    weight_ton: 180,
    price_per_unit: 1800,
    is_active: false,
    actions: { can_update: true, can_delete: true, can_change_status: true }
  },
  {
    ...
    max_distance_km: 120,
    material_type: 'اسمنت',
    truck_type: 'شاحنة صغيرة',
    truck_number: 'ي ك ل 3456',
    weight_ton: 100,
    price_per_unit: 1200,
    is_active: true,
    actions: { can_update: true, can_delete: true, can_change_status: true }
  },
  {
    ...
    max_distance_km: 150,
    material_type: 'حديد',
    truck_type: 'قلاب كبير',
    truck_number: 'م ن س 7890',
    weight_ton: 250,
    price_per_unit: 3500,
    is_active: true,
    actions: { can_update: true, can_delete: true, can_change_status: true }
...
const allHeaders: TableHeader[] = [
  ...
  { key: 'material_type', title: 'نوع المواد' },
  { key: 'truck_type', title: 'نوع الشاحنات' },
  { key: 'truck_number', title: 'رقم الشاحنة' },
  { key: 'weight_ton', title: 'الوزن بالطن' },
  { key: 'price_per_unit', title: 'السعر/الوحدة' },
...
const fetchData = async () => {
  ...
  if (filterTruckType.value) {
    filteredData = filteredData.filter(item => 
      item.truck_type.includes(filterTruckType.value)
    );
  }
...
const handleStatusChange = (item: CustomPricing) => {
...
const handleSelectItem = (item: CustomPricing, selected: boolean) => {
  if (selected) {
...
<template>
  <default-layout>
    <div class="custom-pricing-page">
      <PageHeader :icon="pricingIcon" title-key="pages.customPricing.title"
        description-key="pages.customPricing.description" />
      ...
      <div class="bg-gray-50 rounded-md -mx-6">
          ...
          <div class="flex flex-wrap gap-3 justify-between">
            <div class="flex gap-3 flex-wrap">
              ...
              <TextInput v-model="filterMaterialType" density="comfortable" variant="outlined" hide-details
                placeholder="نوع المواد" class="w-full sm:w-40 bg-white" @keyup.enter="applyFilters" />
              <TextInput v-model="filterTruckType" density="comfortable" variant="outlined" hide-details
                placeholder="نوع الشاحنات" class="w-full sm:w-40 bg-white" @keyup.enter="applyFilters" />
            </div>
        ...
        <DataTable :headers="tableHeaders" :items="tableItems" :loading="loading" :show-checkbox="canBulkDelete" show-actions
          ...
          <template #item.price_per_unit="{ item }">
            <span class="font-medium">{{ item.price_per_unit }} ريال</span>
          </template>
    ...
    <DeleteConfirmDialog v-model="showDeleteDialog" :loading="deleteLoading" :title="$t('pages.customPricing.deleteDialog.title')"
      :message="$t('pages.customPricing.deleteDialog.message', { count: selectedItems.length })" @confirm="confirmBulkDelete" />
