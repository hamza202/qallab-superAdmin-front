...
const fetchData = async (append = false) => {
    try {
        ...
        const response = await api.get<any>(`/zone-ton-pricings?${params.toString()}`);
...
const handleView = (item: any) => {
    router.push(`/settings/pricing-per-ton/view/${item.id}`);
};

const handleDelete = async (item: any) => {
    try {
        await api.delete(`/zone-ton-pricings/${item.id}`);
...
const confirmStatusChange = async () => {
    ...
    try {
        ...
        await api.patch(`/zone-ton-pricings/${itemToChangeStatus.value.id}/change-status`, { status: newStatus });
...
const confirmBulkDelete = async () => {
    ...
    try {
        ...
        await api.post('/zone-ton-pricings/bulk-delete', { ids: selectedItems.value });
...
<template>
    <default-layout>
        <div class="pricing-per-ton-page">
            <PageHeader :icon="SettingsIcon" title-key="التسعير بالطن"
                description-key="إدارة أسعار الشحن حسب الطن والمسافة" />
