            ...
            <div v-else>
                ...
                <div class="flex items-center gap-3 mb-6 text-sm px-6">
                    ...
                    <router-link to="/settings/services-categories" class="text-gray-600 hover:text-primary-600">
                        {{ t('pages.ServicesCategories.title') }}
                    </router-link>
                    <span class="text-lg text-gray-300">/</span>
                ...
                <div class="p-6 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-primary-900 mb-6">{{ t('pages.ServicesCategories.view.generalInfo') }}</h2>
                ...
                <!-- Tax Rules Section -->
                <div class="p-6 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-primary-900 mb-6">{{ t('pages.ServicesCategories.view.addedTaxes') }}</h2>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-base font-semibold" :class="tax.is_active ? 'text-green-600' : 'text-gray-400'">
                                        {{ tax.priority_label || (tax.is_active ? t('pages.ServicesCategories.view.high') : t('common.status.inactive')) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <!-- Attachments Section -->
                <div class="p-6">
                    <h2 class="text-xl font-bold text-primary-900 mb-6">{{ t('pages.ServicesCategories.view.attachments') }}</h2>
                    
                    <div v-if="category.attachments && category.attachments.length > 0" class="space-y-3">
                        <p class="text-sm text-gray-600 mb-4">{{ t('pages.ServicesCategories.view.attachedDocuments') }}</p>
                        <div v-for="attachment in category.attachments" :key="attachment.id" 
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ attachment.size || '200 KB' }} — {{ attachment.uploaded ? `100% ${t('pages.ServicesCategories.view.uploaded')}` : t('pages.ServicesCategories.view.uploading') }}</p>
                                </div>
                            </div>
                    ...
                    <div v-else class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                        ...
                        <p class="text-gray-500">{{ t('pages.ServicesCategories.view.noAttachments') }}</p>
                    </div>
                </div>
            </div>
        </div>
...
const fetchCategory = async () => {
    try {
        ...
        const errorMessage = err.response?.data?.message || err.message || t('common.messages.general.loadDataFailed')
