...
// Categories Data
// =====================
const categoriesList = ref<Array<{ title: string; value: number | string }>>([{ title: "بدون تصنيف رئيسي", value: "" }]);
...
const fetchConstants = async () => {
  try {
    if (USE_MOCK_DATA) {
      ...
      const response = await api.get('/admin/api/categories/constants');

      // Populate taxes dropdown
      taxNameItems.value = [
        { title: "اختر الضريبة", value: "" },
        ...response.data.taxes.map((tax: any) => ({
      ...
      unitItems.value = [
        { title: "اختر الوحدة", value: "" },
        ...response.data.units.map((unit: any) => ({
          title: unit.name,
          value: unit.id,
        })),
      ];
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEditing.value && categoryId.value) {
      ...
      if (USE_MOCK_DATA) {
        console.log('Mock: Updating category', categoryId.value, payload);
      } else {
        ...
        if (payload.image) {
          ...
          await api.upload(`/admin/api/categories/${categoryId.value}`, formData);
        } else {
          await api.put(`/admin/api/categories/${categoryId.value}`, payload);
        }
      }
...
onMounted(async () => {
  ...
  // Check if editing mode from route query
      ...
      <v-form ref="formRef" v-model="isFormValid" @submit.prevent>
        <div class="space-y-6">
          ...
          <div class="bg-gray-50 rounded-md p-4 sm:p-6">
            ...
            <div class="grid gap-4 mb-4 grid-cols-1 md:grid-cols-[repeat(auto-fit,minmax(250px,1fr))]">
              ...
              <SelectWithIconInput v-model="parentCategory" label="التصنيف الرئيسي" placeholder="اختر التصنيف الرئيسي"
                :items="categoriesList" :hide-details="false" />
          ...
          <div class="flex justify-center gap-3 mt-6 sm:flex-row flex-col">
            <!-- Save Button -->
            <v-btn variant="flat" color="primary" height="48" class="font-semibold text-base px-10"
              prepend-icon="mdi-content-save-all-outline" :loading="isSaving" :disabled="isSaving || isDeleting"
              @click="handleSave">
              {{ isEditing ? 'تحديث' : 'حفظ' }}
            </v-btn>
            ...
            <v-btn variant="flat" color="primary-100" height="48"
              class="font-semibold text-base text-primary-700 px-10" prepend-icon="mdi-close"
