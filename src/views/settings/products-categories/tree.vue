...
const fetchConstants = async () => {
  try {
    ...
    toast.error(t('pages.ProductsCategories.tree.messages.loadDataFailed'));
  }
};
...
const fetchCategoriesTree = async () => {
  ...
  try {
    ...
    console.error('Failed to fetch categories:', error);
    toast.error(t('pages.ProductsCategories.tree.messages.loadCategoriesFailed'));
  } finally {
...
const fetchCategoryDetails = async (id: number) => {
  ...
  try {
    ...
    toast.error(t('pages.ProductsCategories.tree.messages.loadCategoryFailed'));
  } finally {
    isLoading.value = false;
  }
};
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isBulkMode.value) {
      ...
      toast.success(t('pages.ProductsCategories.tree.messages.bulkTaxApplied', { count: selectedCategoryIds.value.length }));
      ...
      if (isEditing.value && selectedCategory.value) {
        ...
        toast.success(t('pages.ProductsCategories.tree.messages.updated'));
      } else {
        // Create new category
        if (payload.image) {
          const formData = new FormData();
        ...
        toast.success(isSubcategoryMode.value ? t('pages.ProductsCategories.tree.messages.createdSub') : t('pages.ProductsCategories.tree.messages.created'));
      }
    }
    ...
    if (err?.response?.status === 422 && err?.response?.data?.errors) {
      ...
      toast.error(err?.response?.data?.message || t('pages.ProductsCategories.tree.messages.validationFailed'));
    } else {
      toast.error(err?.response?.data?.message || t('pages.ProductsCategories.tree.messages.saveFailed'));
    }
  } finally {
...
const confirmBulkDelete = async () => {
  ...
  try {
    ...
    toast.success(t('pages.ProductsCategories.tree.messages.bulkDeleted', { count: selectedCategoryIds.value.length }));
    ...
    toast.error(t('pages.ProductsCategories.tree.messages.bulkDeleteFailed'));
  } finally {
...
const confirmDelete = async () => {
  ...
  try {
    ...
    toast.success(t('pages.ProductsCategories.tree.messages.deleted'));
    ...
    toast.error(t('pages.ProductsCategories.tree.messages.deleteFailed'));
  } finally {
    isDeleting.value = false;
  }
};
  ...
  <default-layout>
      ...
      <div class="flex items-center flex-wrap justify-between">
        ...
        <div class="flex flex-wrap gap-3 mb-6">
          ...
          <ButtonWithIcon v-if="showAddCategoryButton" variant="flat" color="primary" height="44" rounded="4"
            custom-class="font-semibold text-base px-8" :label="t('pages.ProductsCategories.tree.buttons.addCategory')" prepend-icon="mdi-plus-circle-outline"
          ...
          <ButtonWithIcon v-if="showAddSubcategoryButton" variant="flat" rounded="4" color="primary-50" height="44"
            custom-class="font-semibold text-base text-primary-700 px-8" :label="t('pages.ProductsCategories.tree.buttons.addSubcategory')"
      ...
      <v-form ref="formRef" v-model="isFormValid" @submit.prevent>
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div class="bg-white rounded-lg shadow-sm">
            <!-- Header -->
            <div class="px-6">
              <h2 class="text-base font-bold text-gray-900 mb-1">{{ t('pages.ProductsCategories.tree.sidebar.title') }}</h2>
              <p class="text-xs text-gray-600">{{ t('pages.ProductsCategories.tree.sidebar.subtitle') }}</p>
            </div>
            ...
            <template v-else>
              ...
              <div class="px-6 pt-4">
                <TextInput v-model="categoriesSearch" density="comfortable" variant="outlined" hide-details
                  :placeholder="t('pages.ProductsCategories.tree.sidebar.searchPlaceholder')" class="mb-4 !text-gray-500" bg-color="gray-50" rounded="lg">
              ...
              <div class="px-6 pb-4 max-h-[600px] overflow-y-auto space-y-2">
                ...
                <div v-if="!filteredCategories.length" class="text-center text-gray-500 py-8">
                  {{ t('pages.ProductsCategories.tree.sidebar.emptyCategories') }}
                </div>
              </div>
            </template>
          </div>
          ...
          <div class="xl:col-span-2 space-y-6">
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h3 class="text-base font-bold text-primary-800">
                    {{ t('pages.ProductsCategories.tree.bulkBanner.title', { count: selectedCategoryIds.length }) }}
                  </h3>
                  <p class="text-sm text-primary-600">
                    {{ t('pages.ProductsCategories.tree.bulkBanner.subtitle') }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div v-if="!isBulkMode" class="bg-gray-50 rounded-md p-4 sm:p-6">
              <div class="mb-4">
                <h2 class="text-lg font-bold text-primary-900">{{ t('pages.ProductsCategories.tree.formPanel.title') }}</h2>
              </div>

              <div class="grid gap-6 mb-4 grid-cols-1 md:grid-cols-2">
                <LanguageTabs :languages="availableLanguages" :label="t('pages.ProductsCategories.tree.labels.name')">
                  <template #en>
                    <TextInput v-model="categoryNameEn" :placeholder="t('pages.ProductsCategories.tree.placeholders.nameEn')"
                  ...
                  <template #ar>
                    <TextInput v-model="categoryNameAr" :placeholder="t('pages.ProductsCategories.tree.placeholders.nameAr')"
                      :rules="[required(), minLength(2), maxLength(100)]" :hide-details="false"
                      :error-messages="formErrors['name.ar']" @input="delete formErrors['name.ar']" />
                  </template>
                </LanguageTabs>

                <SelectWithIconInput show-add-button v-model="parentCategory" :label="t('pages.ProductsCategories.tree.labels.parentCategory')"
                  :placeholder="t('pages.ProductsCategories.tree.placeholders.parentCategory')" :items="CategoryDropdownItems" :hide-details="false"
                  :disabled="isSubcategoryMode" clearable v-if="isSubcategoryMode" />

                <SelectWithIconInput show-add-button v-model="unit" :label="t('pages.ProductsCategories.tree.labels.unit')" :placeholder="t('pages.ProductsCategories.tree.placeholders.unit')"
                ...
                <div>
                  <span class="text-sm font-semibold text-gray-700 mb-2 block">{{ t('pages.ProductsCategories.tree.labels.status') }}</span>
                  <div class="flex items-center gap-3 mt-1">
                    <v-radio-group v-model="isActive" inline hide-details>
                      <v-radio :value="true" color="primary">
                        <template #label>
                          <span :class="isActive ? 'text-primary font-semibold' : 'text-gray-600'">
                            {{ t('pages.ProductsCategories.tree.labels.active') }}
                          </span>
                        </template>
                      </v-radio>
                      <v-radio :value="false" color="primary">
                        <template #label>
                          <span :class="!isActive ? 'text-primary font-semibold' : 'text-gray-600'">
                            {{ t('pages.ProductsCategories.tree.labels.inactive') }}
                          </span>
                        </template>
                      </v-radio>
                    </v-radio-group>
              ...
              <div>
                ...
                <LanguageTabs :languages="availableLanguages" :label="t('pages.ProductsCategories.tree.labels.categoryDetails')" class="mb-[20px]">
                  <template #en>
                    <TextareaInput :rules="[required()]" v-model="categoryDescriptionEn" :placeholder="t('pages.ProductsCategories.tree.placeholders.descriptionEn')"
                      min-height="120px" :hide-details="false"
                      :error-messages="formErrors['description.en']" @input="delete formErrors['description.en']" />
                  </template>
                  <template #ar>
                    <TextareaInput :rules="[required()]" v-model="categoryDescriptionAr" :placeholder="t('pages.ProductsCategories.tree.placeholders.descriptionAr')"
                      min-height="120px" :hide-details="false"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <FileUploadInput v-model="categoryImage" :label="t('pages.ProductsCategories.tree.labels.attachImage')" :hint="t('pages.ProductsCategories.tree.labels.imageHint')"
                  :max-files="1" />
              </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-md p-4 sm:p-6">
              <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-bold text-primary-900">
                  {{ isBulkMode ? t('pages.ProductsCategories.tree.labels.taxesBulkTitle') : t('pages.ProductsCategories.tree.labels.taxesTitle') }}
                </h2>
                <div class="w-auto flex justify-start lg:justify-end">
                  <ButtonWithIcon variant="flat" color="primary" height="40"
                    custom-class="font-semibold text-sm px-4 w-full lg:w-auto" :label="t('pages.ProductsCategories.tree.buttons.addTax')"
              ...
              <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6">
                <div class="w-full lg:w-auto lg:flex-1 min-w-[250px]">
                  <SelectWithIconInput v-model="newTaxRule.tax_id" :label="t('pages.ProductsCategories.tree.labels.tax')" :placeholder="t('pages.ProductsCategories.tree.placeholders.selectTax')"
                ...
                <div class="w-full sm:flex-1 lg:w-auto min-w-[100px]">
                  <TextInput v-model="newTaxRule.percentage" :label="t('pages.ProductsCategories.tree.labels.percentage')" :placeholder="t('pages.ProductsCategories.tree.placeholders.taxDerived')"
                    :hide-details="false" readonly disabled />
                </div>

                <div class="w-full sm:flex-1 lg:w-auto min-w-[140px]">
                  <TextInput v-model="newTaxRule.minValue" :label="t('pages.ProductsCategories.tree.labels.minValue')" :placeholder="t('pages.ProductsCategories.tree.placeholders.taxDerived')"
                    :hide-details="false" readonly disabled />
                </div>

                <div class="w-full sm:flex-1 lg:w-auto min-w-[140px]">
                  <SelectWithIconInput v-model="newTaxRule.priority" :label="t('pages.ProductsCategories.tree.labels.priority')" :placeholder="t('pages.ProductsCategories.tree.placeholders.selectPriority')"
                    :items="priorityItems" :hide-details="false" />
                </div>
              </div>
              <p v-if="formErrors['taxes'] && newTaxRule.tax_id" class="text-xs text-error-500">{{ t('pages.ProductsCategories.tree.taxError') }}</p>
              <div
                ...
                <h3 class="text-base sm:text-lg font-bold text-gray-900 px-6 py-5">
                  {{ t('pages.ProductsCategories.tree.labels.appliedTaxesTitle') }}
                </h3>

                <v-table class="rounded-none overflow-hidden border border-gray-200">
                  <thead>
                    <tr class="bg-gray-50 text-gray-600">
                      <th class="!font-bold">{{ t('pages.ProductsCategories.tree.labels.tax') }}</th>
                      <th class="!font-bold">{{ t('pages.ProductsCategories.tree.labels.percentage') }}</th>
                      <th class="!font-bold">{{ t('pages.ProductsCategories.tree.labels.minValue') }}</th>
                      <th class="!font-bold">{{ t('pages.ProductsCategories.tree.labels.priority') }}</th>
                      <th class="text-center w-[120px] !font-bold">{{ t('pages.ProductsCategories.tree.labels.actions') }}</th>
                    </tr>
                  </thead>
                  <tbody class="bg-white">
                    ...
                    <tr v-if="!taxRules.length">
                      <td colspan="5" class="text-center text-gray-500 py-4">
                        {{ t('pages.ProductsCategories.tree.taxTable.empty') }}
                      </td>
                    </tr>
                  </tbody>
                </v-table>
            ...
            <div class="flex justify-center gap-3 mt-6 sm:flex-row flex-col">
              ...
              <ButtonWithIcon variant="flat" color="primary" height="48" rounded="4"
                ...
                :label="isBulkMode ? t('pages.ProductsCategories.tree.buttons.applyToSelected') : (isEditing ? t('pages.ProductsCategories.tree.buttons.update') : t('pages.ProductsCategories.tree.buttons.save'))" @click="handleSave"
              ...
              <ButtonWithIcon variant="flat" color="primary-50" height="48" rounded="4"
                ...
                :label="isBulkMode ? t('pages.ProductsCategories.tree.buttons.clearSelection') : t('pages.ProductsCategories.tree.buttons.close')" prepend-icon="mdi-close"
              ...
              <ButtonWithIcon v-if="isBulkMode" variant="flat" color="error-50" height="48" rounded="4"
                ...
                :label="t('pages.ProductsCategories.tree.buttons.bulkDelete')" @click="openBulkDeleteDialog" :loading="isDeleting"
              ...
              <ButtonWithIcon v-if="isEditing && selectedCategory && !isBulkMode" variant="flat" color="error-50"
                ...
                :prepend-icon="deleteIcon" :label="t('pages.ProductsCategories.tree.buttons.delete')" @click="openDeleteDialog" :loading="isDeleting"
                :disabled="isSaving || isDeleting" />
            </div>
          </div>
    ...
    <DeleteConfirmDialog v-model="showDeleteDialog" :loading="isDeleting" :persistent="true" @confirm="confirmDelete">
      <p class="text-gray-700 text-center">
        {{ t('pages.ProductsCategories.tree.deleteDialog.intro') }}
        <strong class="text-primary-700">{{ selectedCategory?.name }}</strong>{{ t('pages.ProductsCategories.tree.deleteDialog.outro') }}
      </p>
      <p v-if="selectedCategory?.children?.length" class="text-error-600 text-sm mt-2 text-center">
        ...
        {{ t('pages.ProductsCategories.tree.deleteDialog.childrenWarning') }}
      </p>
    </DeleteConfirmDialog>
