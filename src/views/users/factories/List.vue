...
const factoriesIcon = `<svg width="47" height="42" viewBox="0 0 47 42" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M7.13333 18.8333L13.4167 18.8333L13.4167 8.43333C13.4167 6.00644 13.4167 4.79299 13.889 3.86604C14.3044 3.05067 14.9673 2.38776 15.7827 1.97231C16.7097 1.5 17.9231 1.5 20.35 1.5L25.9833 1.5C28.4102 1.5 29.6237 1.5 30.5506 1.97231C31.366 2.38776 32.0289 3.05067 32.4444 3.86604C32.9167 4.79299 32.9167 6.00644 32.9167 8.43333V18.8333L39.2 18.8333C40.4134 18.8333 41.0202 18.8333 41.4836 19.0695C41.8913 19.2772 42.2228 19.6087 42.4305 20.0164C42.6667 20.4798 42.6667 21.0866 42.6667 22.3L42.6667 40.5H32.9167L13.4167 40.5H3.66667L3.66667 22.3C3.66667 21.0866 3.66667 20.4798 3.90282 20.0164C4.11054 19.6087 4.442 19.2772 4.84969 19.0695C5.31316 18.8333 5.91989 18.8333 7.13333 18.8333Z" fill="#EFF8FF"/>
<path d="M13.4167 18.8333L7.13333 18.8333C5.91989 18.8333 5.31316 18.8333 4.84969 19.0695C4.442 19.2772 4.11054 19.6087 3.90282 20.0164C3.66667 20.4798 3.66667 21.0866 3.66667 22.3L3.66667 40.5M32.9167 18.8333L39.2 18.8333C40.4134 18.8333 41.0202 18.8333 41.4836 19.0695C41.8913 19.2772 42.2228 19.6087 42.4305 20.0164C42.6667 20.4798 42.6667 21.0866 42.6667 22.3L42.6667 40.5M32.9167 40.5L32.9167 8.43333C32.9167 6.00644 32.9167 4.79299 32.4444 3.86604C32.0289 3.05067 31.366 2.38776 30.5506 1.97231C29.6237 1.5 28.4102 1.5 25.9833 1.5L20.35 1.5C17.9231 1.5 16.7097 1.5 15.7827 1.97231C14.9673 2.38776 14.3044 3.05067 13.889 3.86604C13.4167 4.79299 13.4167 6.00644 13.4167 8.43333L13.4167 40.5M44.8333 40.5L1.5 40.5M21 10.1667H25.3333M21 18.8333H25.3333M21 27.5H25.3333" stroke="#1570EF" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
</svg>`;
...
const selectedFactories = ref<number[]>([]);
const hasSelectedFactories = computed(() => selectedFactories.value.length > 0);

const showAdvancedFilters = ref(false);
const filterProductionLines = ref("");
const filterFactoryType = ref<string | null>(null);
...
const factoryTypeItems = ref<Array<{ title: string; value: string }>>([]);
...
const itemToChangeStatus = ref<Factory | null>(null);
...
const fetchFactories = async (cursor?: string | null, append = false) => {
    try {
        if (append) {
        ...
        if (filterFactoryType.value) filters.factory_type = filterFactoryType.value;
        ...
        const url = queryString ? `/factories?${queryString}` : '/factories';

        const response = await api.get<FactoriesResponse>(url);

        const items = Array.isArray(response.data) ? response.data : [];
        const normalizeItem = (item: any) => ({
            ...item,
            actions: item.actions ?? { can_change_status: true, can_update: true, can_delete: true, can_view: true },
        });

        if (append) {
            tableItems.value = [...tableItems.value, ...items.map(normalizeItem)];
        } else {
            tableItems.value = items.map(normalizeItem);
            allHeaders.value = response.headers.filter((h: any) => h.key !== 'id' && h.key !== 'actions');
            shownHeaders.value = response.shownHeaders.filter((h: any) => h.key !== 'id' && h.key !== 'actions');
...
const handleEdit = (item: any) => {
    router.push({ name: "FactoriesEdit", params: { id: item.id } });
};
...
const confirmStatusChange = async () => {
    ...
    try {
        ...
        const currentStatus = itemToChangeStatus.value.is_active ?? itemToChangeStatus.value.status;
        const newStatus = !currentStatus;

        await api.patch(`/factories/${itemToChangeStatus.value.id}/change-status`, {
            status: newStatus
        });
...
const handleBulkDelete = () => {
    if (selectedFactories.value.length === 0) return;
...
const confirmBulkDelete = async () => {
    try {
        ...
        console.error('Error bulk deleting factories:', err);
...
const handleSelectFactory = (item: any, selected: boolean) => {
    if (selected) {
        selectedFactories.value.push(item.id);
    } else {
        selectedFactories.value = selectedFactories.value.filter((id) => id !== item.id);
    }
};

const handleSelectAllFactories = (checked: boolean) => {
    if (checked) {
        selectedFactories.value = tableItems.value.map((item) => item.id);
    } else {
        selectedFactories.value = [];
    }
};
...
const fetchConstants = async () => {
    try {
        ...
        const response = await api.get<{
            ...
            message: string;
            data: {
                factory_types?: Array<{ key: string; label: string }>;
                crusher_types?: Array<{ key: string; label: string }>;
            };
        }>('/factories/constants');

        const types = response.data?.factory_types ?? response.data?.crusher_types;
        if (types && Array.isArray(types)) {
            factoryTypeItems.value = types.map((ct: { key: string; label: string }) => ({
                title: ct.label,
                value: ct.key
            }));
        }
    ...
    observer.value = new IntersectionObserver(
        (entries) => {
            ...
            if (entry?.isIntersecting && hasMoreData.value && !loadingMore.value && !loading.value) {
                fetchFactories(nextCursor.value, true);
            }
        },
    ...
    <default-layout>
        <div class="factories-page">
            ...
            <div class="bg-gray-50 rounded-md -mx-6">
                <div :class="hasSelectedFactories ? 'justify-between' : 'justify-end'"
                ...
                <DataTable :headers="tableHeaders" :items="tableItems" :loading="loading" :show-checkbox="canBulkDelete"
                    show-actions @edit="handleEdit" @delete="confirmDelete" @select="handleSelectFactory"
                    @selectAll="handleSelectAllFactories">
                    <template #item.status_value="{ item }">
                        <v-switch :model-value="!!(item.is_active ?? item.status)" hide-details inset density="compact" color="primary"
                    ...
                    <template #item.activity_types_label="{ item }">
                        {{ Array.isArray(item.activity_types_label) ? item.activity_types_label.join(' / ') : (item.activity_types_label || '--') }}
                    </template>
                </DataTable>
        ...
        <StatusChangeDialog v-model="showStatusChangeDialog" :loading="statusChangeLoading"
            :item-name="itemToChangeStatus?.business_name" :current-status="!!(itemToChangeStatus?.is_active ?? itemToChangeStatus?.status)"
