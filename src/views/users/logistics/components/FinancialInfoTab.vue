...
interface Props {
  bankAccounts?: LogisticBankAccount[];
...
const emit = defineEmits<{
  'update:formData': [data: { bankAccounts: LogisticBankAccount[]; debitLimit?: string; creditLimit?: string }];
...
const bankAccounts = ref<LogisticBankAccount[]>(props.bankAccounts || []);
...
<template>
  <div class="mb-6 bg-gray-50 -mx-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 px-6 pt-6 pb-8">
      <TextInput v-model="creditLimit" @input="emitUpdate" :label="$t('form.financial.creditLimit.label')" :placeholder="$t('form.financial.creditLimit.placeholder')"
      ...
      <TextInput v-model="debitLimit" @input="emitUpdate" :label="$t('form.financial.debitLimit.label')" :placeholder="$t('form.financial.debitLimit.placeholder')"
        ...
        <tr v-for="(account, index) in bankAccounts" :key="index" class="border-b border-gray-200">
          ...
          <td class="py-3 px-4">
            <v-switch v-model="account.is_active" @update:model-value="emitUpdate" hide-details inset density="compact"
              color="primary-600" class="small-switch" />
          </td>
          <td class="py-3 px-4">
