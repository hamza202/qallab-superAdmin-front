...
const pageTitle = computed(() => isEditing.value ? "تعديل تاجر المواد" : "إضافة تاجر مواد");
...
const handleSave = async () => {
  try {
    ...
    if (err?.response?.status === 422 && err?.response?.data?.errors) {
      ...
      toast.error(err?.response?.data?.message || 'فشل حفظ تاجر المواد');
    }
  } finally {
    saving.value = false;
  }
...
const fetchMaterialMerchantData = async () => {
  ...
  try {
    ...
    toast.error(err?.response?.data?.message || 'فشل تحميل بيانات تاجر المواد');
  }
};
