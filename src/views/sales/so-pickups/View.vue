    ...
    <div class="pickup-view-page -mx-6">
      ...
      <div v-else>
        ...
        <div class="flex flex-wrap items-center gap-3 mb-6 text-sm px-6">
          ...
          <span class="text-lg text-gray-300">/</span>
          <span class="text-primary-700 font-medium bg-primary-50 px-2 py-1 rounded-md" dir="ltr">#{{ detail?.code ??
            pickupId }}</span>
        </div>
        ...
        <div class="p-6">
          <h2 class="text-xl font-bold text-primary-900 mb-6">{{ t('pages.SalesSoPickups.viewPage.orderInfo.title') }}
          </h2>

          <div class="flex flex-wrap gap-4">
            <div class="info-item-bordered px-6 py-4 flex-1">
              <label class="font-semibold text-sm text-gray-500 mb-2 block">{{
                t('pages.SalesSoPickups.viewPage.orderInfo.date') }}</label>
              <p class="text-base font-semibold text-gray-800">{{ formatDate(detail?.so_pickup_date) }}</p>
            </div>
            <v-divider vertical class="my-6"></v-divider>
            <div class="info-item-bordered px-6 py-4 flex-1">
              <label class="font-semibold text-sm text-gray-500 mb-2 block">{{
                t('pages.SalesSoPickups.viewPage.orderInfo.loadingResponsible') }}</label>
              <p class="text-base font-semibold text-gray-800">{{ detail?.loading_responsible_party ?? '—' }}</p>
            </div>
            <v-divider vertical class="my-6"></v-divider>
            <div class="info-item-bordered px-6 py-4 flex-1">
              <label class="font-semibold text-sm text-gray-500 mb-2 block">{{
                t('pages.SalesSoPickups.viewPage.orderInfo.unloadingResponsible') }}</label>
              <p class="text-base font-semibold text-gray-800">{{ detail?.downloading_responsible_party ?? '—' }}</p>
            </div>
            <v-divider vertical class="my-6"></v-divider>
            <div class="info-item-bordered px-6 py-4 flex-1">
              <label class="font-semibold text-sm text-gray-500 mb-2 block">{{
                t('pages.SalesSoPickups.viewPage.orderInfo.tripTiming') }}</label>
              <p class="text-base font-semibold text-gray-800">{{ tripPeriodLabel }}</p>
            </div>
            <v-divider vertical class="my-6"></v-divider>
            <div class="info-item-bordered px-6 py-4 flex-1">
              <label class="font-semibold text-sm text-gray-500 mb-2 block">{{
                t('pages.SalesSoPickups.viewPage.orderInfo.notes') }}</label>
        ...
        <div class="mb-8">
          <div class="bg-primary-50 px-6 py-3">
            <div class="flex items-center gap-2">
              ...
              <h2 class="text-lg font-bold text-qallab-blue-light">{{
                t('pages.SalesSoPickups.viewPage.productsTable.title') }}</h2>
            </div>
          </div>
          <div class="mb-4">
            <DataTable :headers="productsHeaders" :items="productsTableData" :show-actions="false"
              cell-class="text-sm text-gray-900" />
          </div>
        </div>
      </div>
    </div>
...
const productsHeaders = computed(() => [
  ...
  { title: t('pages.SalesSoPickups.viewPage.productsTable.quantity'), key: 'quantity', width: '120px' },
  { title: t('pages.SalesSoPickups.viewPage.productsTable.unit'), key: 'unit', width: '120px' },
...
const productsTableData = computed(() => {
  ...
  return products.map((item: any, index: number) => ({
    ...
    unit: item.unit_name ?? item.unit ?? item.unit_id ?? '—',
