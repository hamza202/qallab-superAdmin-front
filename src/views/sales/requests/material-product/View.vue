            ...
            <div>
                ...
                <div class="flex flex-wrap items-center gap-3 mb-6 text-sm px-6">
                    ...
                    <router-link to="/sales/requests/material-product/list"
                        ...
                        {{ t('sales.forms.viewPages.lists.materialProductQuotationRequests') }}
                    </router-link>
                    <span class="text-lg text-gray-300">/</span>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600">{{ t('sales.forms.viewPages.subtitles.materialProductRequestIntro') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <v-tabs-window v-model="activeTab">
                    <v-tabs-window-item :value="0">
                        <div class="p-6 border-b !border-gray-200">
                            <h2 class="text-xl font-bold text-primary-900 mb-6">{{ t('sales.forms.viewPages.sections.orderRequestInfo') }}</h2>
                            ...
                            <div class="flex flex-wrap gap-4">
                                ...
                                <div class="info-item-bordered flex-1 px-6 py-4">
                                    <label class="font-semibold text-sm text-gray-500 mb-2 block">{{ t('sales.forms.common.labels.supplierName') }}</label>
                                ...
                                <v-divider vertical class="my-6"></v-divider>
                                <div class="info-item-bordered flex-1 px-6 py-4">
                                    <label class="font-semibold text-sm text-gray-500 mb-2 block">{{ t('sales.forms.common.labels.requestIssueDate') }}</label>
                                    <p class="text-base font-semibold text-gray-900">{{ requestDateTime }}</p>
                                </div>
                                <v-divider vertical class="my-6"></v-divider>
                                <div class="info-item-bordered flex-1 px-6 py-4">
                                    <label class="font-semibold text-sm text-gray-500 mb-2 block">{{ t('sales.forms.common.labels.requestStatus') }}</label>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 justify-between lg:gap-20">
                            <div class=" p-6">
                                <div
                                    ...
                                    <h3 class="text-lg font-bold">{{ t('sales.forms.viewPages.sections.attachmentsHeading') }}</h3>
                                </div>

                                <div class="rounded-xl bg-primary-50 px-4 py-6 mb-3 flex items-center justify-between">
                                    <div>
                                        <p class="text-primary-600 font-bold text-sm mb-2">{{ t('sales.forms.viewPages.sections.voiceNote') }}</p>
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-3 gap-2">
                                    <div class="rounded-lg border border-gray-100 lg:col-span-2 p-4">
                                        <p class="text-primary-600 font-bold text-sm mb-2">{{ t('sales.forms.viewPages.sections.textNoteHeading') }}</p>
                                        <p class="text-gray-400 font-medium text-sm">
                                            {{ t('sales.forms.common.placeholders.voiceNotesFuelsRequest') }}
                                        </p>
                                    </div>
                            ...
                            <div class=" p-6">
                                <div
                                    ...
                                    <h3 class="text-lg font-bold">{{ t('sales.forms.viewPages.sections.orderSummaryHeading') }}</h3>
                                </div>
                        ...
                        <div class="mb-8">
                            <div class="flex flex-wrap gap-3 items-center justify-between bg-primary-50 px-6 py-3">
                                <div class="flex items-center gap-2 text-primary-900">
                                    ...
                                    <h2 class="text-lg font-bold ">{{ t('sales.forms.common.sections.transportService') }}</h2>
                                </div>
                            </div>
...
const fetchRequestData = async () => {
    ...
    try {
        ...
        error(e?.response?.data?.message || t('sales.forms.common.messages.loadMaterialProductRequestFailed'))
    } finally {
...
const requestType = computed(() => {
    ...
    if (type === 'request_without_logistics') return t('sales.forms.viewPages.requestKind.withoutLogistics')
    if (type === 'request_with_logistics') return t('sales.forms.viewPages.requestKind.withLogistics')
    return type || '--'
})
...
const paymentMethod = computed(() => {
    ...
    if (method === 'cash') return t('sales.forms.paymentsDemo.cashAlt')
...
const productItems = computed(() => {
    ...
    return requestData.value.items.map((item: any) => ({
        ...
        notes: item.notes || t('sales.forms.common.misc.none'),
    }))
})
...
const transportServiceItems = computed(() => {
    ...
    return [{
        ...
        notes: logistics.notes || t('sales.forms.common.misc.none'),
    }]
})
...
const tabs = computed(() => [
    {
        title: t('sales.forms.viewPages.tabs.materialProductsGeneral'),
        value: 0,
    ...
    {
        title: t('sales.forms.viewPages.tabs.transportData'),
        value: 2,
...
const headers = computed(() => [
    { title: t('sales.forms.tables.tripProducts.itemName'), key: 'name' },
...
const ServicesHeaders = computed(() => [
    { title: t('sales.forms.common.labels.projectMap'), key: 'project_location' },
    { title: t('sales.forms.viewPages.sections.dateFromCol'), key: 'from_date' },
    { title: t('sales.forms.viewPages.sections.dateToCol'), key: 'to_date' },
    { title: t('sales.forms.common.labels.vehicleTypes'), key: 'vehicle_type' },
