...
const handleDownloadSignedPo = async (item: { uuid?: string; id?: string | number; signed_po_file_url?: string | null }) => {
  ...
  try {
    ...
    await triggerPdfDownloadFromSignedUrl(url, pathName);
  } catch (err: any) {
...
const handlePrint = (item: { id?: string | number; uuid?: string }) => {
  ...
  const routeData = router.resolve({ name: 'SalesQuotationsLogisticsPrint', params: { id: uuid } });
...
const handleDownloadPdf = async (item: { id?: string | number; uuid?: string }) => {
  ...
  try {
    const body = (await api.get(
      `/sales/quotations/logistics/${uuid}/pdf`
...
const fetchList = async () => {
  ...
  try {
    ...
    error(err?.response?.data?.message || t('sales.quotationsLogistics.messages.loadListError'));
  } finally {
    loading.value = false;
  }
};
...
const handleToggleHeader = async (headerKey: string) => {
  await toggleHeader(headerKey).catch((err: any) => {
    error(err?.response?.data?.message || t('sales.quotationsLogistics.messages.columnsUpdateError'));
  });
};
...
const confirmDelete = async () => {
  ...
  try {
    ...
    success(t('sales.quotationsLogistics.messages.deleteSuccess'));
    showDeleteDialog.value = false;
    itemToDelete.value = null;
    await fetchList();
  } catch (err: any) {
    console.error('Error deleting quotation:', err);
    error(err?.response?.data?.message || t('sales.quotationsLogistics.messages.deleteError'));
  } finally {
...
const confirmBulkDelete = async () => {
  try {
    ...
    success(t('sales.quotationsLogistics.messages.bulkDeleteSuccess', { count: selectedRequests.value.length }));
    selectedRequests.value = [];
    await fetchList();
  } catch (err: any) {
    console.error('Error bulk deleting:', err);
    error(err?.response?.data?.message || t('sales.quotationsLogistics.messages.bulkDeleteError'));
  } finally {
    deleteLoading.value = false;
...
<template>
  <default-layout>
    <div class="pricesOffers-page">
      <PageHeader :icon="GridIcon" title-key="sales.quotationsLogistics.list.title"
        description-key="sales.quotationsLogistics.list.description" />
        ...
        <div :class="hasSelectedRequests ? 'justify-between' : 'justify-end'"
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <ButtonWithIcon v-if="canCreate" variant="flat" color="primary-100" height="40" rounded="4"
              ...
              :prepend-icon="plusIcon" :label="t('sales.quotationsLogistics.addQuotation')" @click="openCreateQuotation" />
          </div>
        </div>
        ...
        <div v-if="showAdvancedFilters"
          ...
          <div class="flex flex-wrap gap-3 items-end">
            <TextInput v-model="filterRequestNumber" density="comfortable" variant="outlined" hide-details
              :placeholder="t('sales.quotationsLogistics.filters.quotationCode')" class="w-full sm:w-40 bg-white" />
            <TextInput v-model="filterNameEnglish" density="comfortable" variant="outlined" hide-details
              :placeholder="t('sales.quotationsLogistics.filters.customerName')" class="w-full sm:w-40 bg-white" />
            <TextInput v-model="filterNameArabic" density="comfortable" variant="outlined" hide-details
              :placeholder="t('sales.quotationsLogistics.filters.price')" class="w-full sm:w-40 bg-white" />
            <DatePickerInput v-model="filterStartDateMin" density="comfortable" hide-details
              :placeholder="t('sales.quotationsLogistics.filters.quotationDate')" class="w-full sm:w-40 bg-white" />
          </div>
          <div class="flex gap-2 items-center">
            <ButtonWithIcon variant="flat" color="primary-500" rounded="4" height="40"
              custom-class="px-5 font-semibold !text-white text-sm sm:text-base" :prepend-icon="searchIcon"
              :label="t('sales.quotationsLogistics.search')" @click="applyFilters" />
    ...
    <DeleteConfirmDialog v-model="showDeleteDialog" :loading="deleteLoading"
      :title="t('sales.quotationsLogistics.delete.titleSingle')"
      :message="t('sales.quotationsLogistics.delete.messageSingle')" @confirm="confirmDelete" />

    <DeleteConfirmDialog v-model="showBulkDeleteDialog" :loading="deleteLoading"
      :title="t('sales.quotationsLogistics.delete.titleBulk')"
      :message="t('sales.quotationsLogistics.delete.messageBulk', { count: selectedRequests.length })"
