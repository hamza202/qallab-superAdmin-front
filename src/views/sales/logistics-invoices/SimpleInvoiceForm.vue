...
interface ProductTableItem {
    ...
    isOriginal?: boolean; // true for existing invoice items, false for newly fetched trips
}
...
const fetchTripsByDateRange = async (fromDate: string | null, toDate: string | null) => {
    if (!fromDate || !toDate) {
    ...
    try {
        const res = await api.get<any>('sales/trips/completed-not-invoiced', {
            params: {
        ...
        productTableItems.value = trips.map((trip: any) => ({
            ...
            isOriginal: false, // Newly fetched trips are not original
        }));
        ...
        if (data) {
            ...
            if (data.items && Array.isArray(data.items)) {
                productTableItems.value = data.items.map((item: any) => ({
                    ...
                    isOriginal: true, // Mark as original invoice item
                }));
            }
...
const buildFormData = (): FormData => {
    ...
    fd.append('created_from', String(formData.value.created_from || ''));
    fd.append('created_to', String(formData.value.created_to || ''));
    fd.append('notes', formData.value.notes || '');

    productTableItems.value.forEach((item, index) => {
        if (isEditMode.value && item.id && item.isOriginal) {
...
const handleSubmit = async (type: any) => {
    ...
    try {
        ...
        if (isEditMode.value) {
            await api.post(`/sales/invoices/logistics/${routeId.value}?type=simple`, fd, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
        } else {
            await api.post('/sales/invoices/logistics/?type=simple', fd, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
        }
...
const deleteTrip = (Trip: { id?: string | number }) => {
    const index = productTableItems.value.findIndex(item => item.trip_id === Trip.id || item.id === Trip.id);
    if (index > -1) {
...
watch(
    () => [formData.value.created_from, formData.value.created_to],
    ([fromDate, toDate]) => {
        if (fromDate && toDate) {
            fetchTripsByDateRange(fromDate, toDate);
        }
    }
);
...
<template>
    <default-layout>
        <div class="-mx-6 bg-qallab-dashboard-bg space-y-4">
            <AppFormBreadcrumb list-path="/sales/logistics-invoices/list" module-root-key="breadcrumb.sales.root"
                list-label-key="breadcrumb.sales.logisticsInvoices.list"
                create-label-key="breadcrumb.sales.logisticsInvoices.create"
                edit-label-key="breadcrumb.sales.logisticsInvoices.edit" :is-edit-mode="isEditMode"
                :code="InvoiceCode" />
            ...
            <div class="p-6 bg-white rounded-3xl border !border-gray-100 ">
                ...
                <v-form ref="formRef" v-model="isFormValid" @submit.prevent>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 gap-y-6">
                        <!-- اسم العميل -->
                        <div>
                            <SelectInput v-model="formData.customer_id" :items="[]"
                                :placeholder="t('sales.forms.common.placeholders.selectClient')"
                                :label="t('sales.forms.common.labels.customerName')" density="comfortable"
                                :rules="[required()]" item-title="title" item-value="value" :server-side="true"
                        ...
                        <!-- Created From Date -->
                        <div>
                            <DatePickerInput v-model="formData.created_from" type="date" density="comfortable"
                                :max="new Date().toISOString().split('T')[0]"
                                :placeholder="t('pages.SalesLogisticsInvoices.form.datePlaceholder')"
                                :label="t('pages.SalesLogisticsInvoices.form.createdFromLabel')" />
                        </div>

                        <!-- Created To Date -->
                        <div>
                            <DatePickerInput v-model="formData.created_to" type="date" density="comfortable"
                                :min="formData.created_from || ''" :max="new Date().toISOString().split('T')[0]"
                                :placeholder="t('pages.SalesLogisticsInvoices.form.datePlaceholder')"
                                :label="t('pages.SalesLogisticsInvoices.form.createdToLabel')" />
                        </div>
                        ...
                        <div v-if="formData.invoice_creation_date">
                            <DatePickerInput v-model="formData.invoice_creation_date" type="date" density="comfortable"
                                :placeholder="t('sales.forms.common.placeholders.dateSample')"
                                :label="t('sales.forms.common.labels.invoiceCreatedAt')" />
                        </div>
                        ...
                        <div v-if="customerData"
                            ...
                            <div>
                                <p class="mb-2 font-semibold">{{ t('sales.forms.common.customerCard.commercialRegister')
                                    }}</p>
                        ...
                        <div>
                            <DateTimePickerInput v-model="formData.invoice_issues_datetime" type="date"
                                density="comfortable" :placeholder="t('sales.forms.common.placeholders.dateSample')"
                                :label="t('sales.forms.common.labels.invoiceIssueDate')" />
                        </div>
                        ...
                        <div>
                            <DateTimePickerInput v-model="formData.invoice_due_datetime" type="date"
                                density="comfortable" :placeholder="t('sales.forms.common.placeholders.dateSample')"
                                :label="t('sales.forms.common.labels.invoiceDueDate')" />
                        </div>


                        <!-- Project -->
                        <div>
                            <TextInput v-model="formData.project_name"
                                :placeholder="t('sales.forms.common.placeholders.enterProjectName')"
                                :label="t('sales.forms.common.labels.project')" density="comfortable" />
                        </div>

                        <!-- Statement (Full width) -->
                        <div class="lg:col-span-2">
                            <TextareaInput v-model="formData.notes"
                                :placeholder="t('sales.forms.common.labels.statementHere')"
                                :label="t('sales.forms.common.labels.statement')" density="comfortable" rows="3" />
                        </div>
                    </div>
                </v-form>
            </div>
            ...
            <div class="bg-white rounded-3xl border !border-gray-100">
                <div class="p-6">
                    <div class="flex items-center gap-2 text-primary-600">
                        ...
                        <h2 class="text-base font-bold ">{{ t('sales.forms.common.sections.salesInvoiceItemsTable') }}
                        </h2>
                    </div>
                </div>

                <!-- Products Table -->
                <DataTable :headers="headers" :items="tableItems" :forceShowDelete="true" @delete="deleteTrip">
